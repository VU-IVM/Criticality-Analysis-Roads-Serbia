...
# def add_distances(network):
#     """
#     Adds a 'distance' column (in meters) to the edges of the network,
#     using an accurate local projection for Georgia (EPSG:32638).

#     Args:
#         network (Network): A network composed of nodes (points in space) and edges (lines).

#     Returns:
#         Network: The same network with updated edge distances.
#     """

#     # Project edges to UTM Zone 38N for Georgia (accurate in meters)
#     projected_edges = network.edges.to_crs(epsg=32638)

#     # Compute length of each edge (in meters)
#     network.edges['distance'] = projected_edges.geometry.length

#     return Network(nodes=network.nodes, edges=network.edges)
...
def add_travel_time(network):
    ...
    def calculate_time(edge):
        ...
        try:
            ...
            return edge['distance'] / (edge['maxspeed'])  # when speed is kilo meters per hour
        except:
...
#     #create a list of all the edge attributes
#     if  'highway' in network.edges.columns:
#         attributes= list(network.edges.columns[:6])

    
#     elif 'railway' in network.edges.columns:
#         attributes= list(network.edges.columns[:6])

#     grab_all_edges = []

#     #iterate over all the edges
#     for edge in tqdm(network.edges.itertuples(index=True),total=len(network.edges)):
...
#             # grab_all_edges.append(pd.DataFrame([[edge.osm_id,edge.highway,edge.name,edge.maxspeed,edge.oneway,edge.lanes,edge.surface,x] for x in new_edges],columns=network.edges.columns))
# #--------------------------------------------------------------------------------------------------------------------   
#             # columns = ['osm_id','highway','name','maxspeed','oneway','lanes','surface','bridge','tunnel','geometry']         
#             grab_all_edges.append(pd.DataFrame([[edge.osm_id,edge.highway,edge.name,edge.maxspeed,edge.oneway,edge.lanes,edge.surface, edge.bridge, edge.tunnel,x] for x in new_edges],columns=network.edges.columns))
#             # grab_all_edges.append(pd.DataFrame([[edge.osm_id,edge.highway,edge.name,edge.maxspeed,edge.oneway,edge.lanes,edge.surface, edge.bridge, edge.tunnel,x] for x in new_edges],columns=columns#network.edges.columns))
# #---------------------------------------------------------------------------------------------------------------------       
#         elif 'railway' in attributes:
#             grab_all_edges.append(pd.DataFrame([[edge.osm_id,edge.railway,edge.name,edge.gauge,edge.electrified,edge.voltage,x] for x in new_edges],columns=network.edges.columns))
...
# #------------------------------------------------------------------------------------------------------------------
#     # combine, convert to GeoDataFrame, making sure of the geometry`s crs
#     edges_df = pd.concat(grab_all_edges).reset_index(drop=True)
#     edges_gdf = gpd.GeoDataFrame(edges_df, geometry='geometry')
#     edges_gdf.set_crs(network.edges.crs, inplace=True)
...
def split_edges_at_nodes(network, tolerance=1e-9):
    ...
    for edge in tqdm(network.edges.itertuples(index=False), desc="split_edges", total=len(network.edges)):
        try:
            # Find nodes that intersect with the edge
            hits_nodes = nodes_intersecting(edge.geometry, network.nodes, sindex_nodes, tolerance=tolerance)
            
            # Find edges that intersect with the current edge
            hits_edges_idx = sindex_edges.query(edge.geometry, predicate='intersects')
            if len(hits_edges_idx) > 0:
                hits_edges_geoms = network.edges.iloc[hits_edges_idx]['geometry']
                hits_edges = shapely.set_operations.intersection(edge.geometry, hits_edges_geoms)
                
                # Filter out edges that completely cover the current edge
                if hasattr(hits_edges, '__len__') and len(hits_edges) > 0:
                    # Convert to list if it's a single geometry
                    if not isinstance(hits_edges, (list, pd.Series)):
                        hits_edges = [hits_edges]
                    
                    # Filter edges and extract points
                    valid_hits = []
                    for hit in hits_edges:
                        if hit is not None and not shapely.predicates.covers(hit, edge.geometry):
                            if shapely.get_type_id(hit) == 0:  # Point
                                valid_hits.append(hit)
                            elif shapely.get_type_id(hit) in [1, 5]:  # LineString or MultiLineString
                                coords = shapely.get_coordinates(hit)
                                for coord in coords:
                                    valid_hits.append(shapely.points(coord))
                    
                    hits_edges_points = pd.Series(valid_hits, name='geometry') if valid_hits else pd.Series([], name='geometry')
                else:
                    hits_edges_points = pd.Series([], name='geometry')
            else:
                hits_edges_points = pd.Series([], name='geometry')
            
            # Combine node hits and edge intersection points
            if not hits_nodes.empty and not hits_edges_points.empty:
                all_hits = pd.concat([hits_nodes['geometry'], hits_edges_points], ignore_index=True)
            elif not hits_nodes.empty:
                all_hits = hits_nodes['geometry']
            elif not hits_edges_points.empty:
                all_hits = hits_edges_points
            else:
                # No intersections, keep original edge
                grab_all_edges.append([[edge.osm_id], [edge.geometry], [edge[1:-1]]])
                continue
            
            # Extract unique points
            if len(all_hits) > 0:
                # Convert all geometries to points if they aren't already
                point_list = []
                for geom in all_hits:
                    if shapely.get_type_id(geom) == 0:  # Point
                        point_list.append(geom)
                    else:
                        # Extract coordinates and create points
                        coords = shapely.get_coordinates(geom)
                        for coord in coords:
                            point_list.append(shapely.points(coord))
                
                if point_list:
                    unique_points = shapely.constructive.extract_unique_points(
                        shapely.multipoints(point_list)
                    )
                    hits = [shapely.points(x) for x in shapely.get_coordinates(unique_points)]
                else:
                    hits = []
            else:
                hits = []
            
            # Snap points to edge and get split locations
            if hits:
                snapped_points = [shapely.snap(hit, edge.geometry, tolerance) for hit in hits]
                split_points = shapely.get_coordinates(shapely.multipoints(snapped_points))
                edge_coords = shapely.get_coordinates(edge.geometry)
                
                # Find split locations along the edge
                split_locs = []
                for split_point in split_points:
                    distances = np.sqrt(np.sum((edge_coords - split_point)**2, axis=1))
                    closest_idx = np.argmin(distances)
                    if distances[closest_idx] < tolerance * 10:  # Allow some tolerance
                        split_locs.append(closest_idx)
                
                # Remove duplicates and sort
                split_locs = sorted(list(set(split_locs)))
                
                # Add start and end indices
                if 0 not in split_locs:
                    split_locs.insert(0, 0)
                if len(edge_coords) - 1 not in split_locs:
                    split_locs.append(len(edge_coords) - 1)
                
                # Create new edge segments
                if len(split_locs) > 2:
                    split_pairs = list(zip(split_locs[:-1], split_locs[1:]))
                    new_edges = []
                    for start_idx, end_idx in split_pairs:
                        if start_idx != end_idx:
                            segment_coords = edge_coords[start_idx:end_idx+1]
                            if len(segment_coords) >= 2:
                                new_edges.append(shapely.linestrings(segment_coords))
                ...
                # No intersection points, keep original
                grab_all_edges.append([[edge.osm_id], [edge.geometry], [edge[1:-1]]])
                
        except Exception as e:
            print(f"Error processing edge {edge.osm_id}: {e}")
            # Keep original edge in case of error
            grab_all_edges.append([[edge.osm_id], [edge.geometry], [edge[1:-1]]])
    ...
    for sublist in big_list:
        for item in sublist:
            row = [item[0], item[1]] + list(item[2])
            edges_data.append(row)
    
    edges = pd.DataFrame(edges_data, columns=['osm_id', 'geometry'] + attributes)
...
# --------------------------------------------------!
# ----------------------------------------------    
#     net = Network(edges=df)#edges=sub_full_network)
#     net = add_endpoints(net)
#     net = add_ids(net)
#     net = add_topology(net)    
