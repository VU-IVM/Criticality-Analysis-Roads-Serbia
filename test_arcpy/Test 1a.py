...
# -----------------------------------------------
arcpy.env.overwriteOutput = True
output_folder = Path(r"C:\Temp\arcgis_tmp")
output_folder.mkdir(parents=True, exist_ok=True)

# GDB and feature class names
gdb_path = str(output_folder / "temp.gdb")
fc_name = "roads_mercator"

# -----------------------------------------------
# CREATE GDB if not exists
# -----------------------------------------------
if not arcpy.Exists(gdb_path):
    arcpy.management.CreateFileGDB(str(output_folder), "temp.gdb")

# -----------------------------------------------
# PROCESS GeoDataFrame (gdf_mercator)
# -----------------------------------------------
# Shorten column names for GDB
gdf_mercator.columns = [c[:10] for c in gdf_mercator.columns]
...
# Write GeoDataFrame to GDB
gdf_mercator.to_file(gdb_path, layer=fc_name, driver="OpenFileGDB")
...
arcpy.AddMessage("Processed layer added to map successfully!")