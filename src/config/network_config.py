...
class NetworkConfig:
    ...
    Path_flood_statistics_per_basin = (
        data_path / "SRB_flood_statistics_per_Basin_basins_scenario.csv"
    )
    ...
    wildfire_risk = (
        data_path / "wildfire risk" / "stepen ugrozenosti od pozara Srbijasume.tif"
    )
    ...
    degree_of_urbanization = (
        data_path / "GHS_SMOD_E2025_GLOBE_R2023A_54009_1000_V2_0_R4_C20.tif"
    )
    ...
    Path_criticality_results = (
        travel_disruptions_parquet / "criticality_results.parquet"
    )
    ...
    Path_future_floods_change_RP = (
        hazard_exposure_parquet / "Future Floods change in RP.parquet"
    )
    Path_future_flooding_roads = (
        hazard_exposure_parquet
        / "Future Floods change in RP experienced by roads.parquet"
    )
    Path_precipitation_change_rcp_45_period_1 = (
        hazard_exposure_parquet
        / "change in maximum daily precipitation rcp 45 period 1.parquet"
    )
    Path_precipitation_change_rcp_45_period_2 = (
        hazard_exposure_parquet
        / "change in maximum daily precipitation rcp 45 period 2.parquet"
    )
    Path_precipitation_change_rcp_85_period_1 = (
        hazard_exposure_parquet
        / "change in maximum daily precipitation rcp 85 period 1.parquet"
    )
    Path_precipitation_change_rcp_8_5_far_future = (
        hazard_exposure_parquet
        / "change in maximum daily precipitation rcp 85 period 2.parquet"
    )

    # heat data and impacts (produced in 4c)
    Future_pavement_temperatures = (
        intermediate_results_path / "future_pavement_temperatures.tif"
    )
    roads_current_max_pavement_temperature = (
        hazard_exposure_parquet / "roads_current_max_pavement_temperatures.parquet"
    )
    roads_future_max_pavement_temperature = (
        hazard_exposure_parquet / "roads_future_max_pavement_temperatures.parquet"
    )
    ...
    Path_main_network_hazard_exposure = (
        hazard_exposure_parquet / "main_network_hazard_exposure.parquet"
    )
    ...
    Path_climate_criticality_results = (
        results_path / "Climate_Criticality_PuteviSrbije.xlsx"
    )
    ...
    ######################################################
    show_figures = (
        False  # Flag to set whether plots will be shown in a pop up window or not
    )
