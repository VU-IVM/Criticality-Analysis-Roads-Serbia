...
class LocalConfig:
    ...
    osm_plot_road_types: List[str] = field(
        default_factory=lambda: [
            "primary",
            "trunk",
            "motorway",
    ...
    serbian_road_categories: List[str] = field(
        default_factory=lambda: ["IA", "IM", "IB", "IIA", "IIB"]
    )

    # Road visualization colors
    osm_road_colors: Dict[str, str] = field(
        default_factory=lambda: {
            "motorway": "#8B0000",
            "trunk": "#1E90FF",
            "primary": "#A52A2A",
    ...
    serbian_road_colors: Dict[str, str] = field(
        default_factory=lambda: {
            "IA": "#8B0000",
            "IM": "#1E90FF",
            "IB": "#A52A2A",
    ...
    osm_road_widths: Dict[str, float] = field(
        default_factory=lambda: {
            "motorway": 3.0,
            "trunk": 2.5,
            "primary": 2.0,
    ...
    serbian_road_widths: Dict[str, float] = field(
        default_factory=lambda: {
            "IA": 3.0,
            "IM": 2.5,
            "IB": 2.0,
    ...
    osm_road_hierarchy: List[str] = field(
        default_factory=lambda: [
            "motorway",
            "trunk",
            "primary",
    ...
    osm_road_labels: Dict[str, str] = field(
        default_factory=lambda: {
...
def load_osm_network(config: LocalConfig) -> gpd.GeoDataFrame:
    ...
    ]  # In OSM, the highway tag identifies roads (motorway, primary, residential, etc.).
    ...
    base_network = features[
        features["highway"].isin(config.osm_road_types)
...
def prepare_osm_network_for_plotting(
    base_network: gpd.GeoDataFrame, config: LocalConfig
) -> gpd.GeoDataFrame:
    """
...
def plot_osm_network(
    base_network_filtered: gpd.GeoDataFrame, config: LocalConfig
) -> None:
    """
    ...
    for category in reversed(config.osm_road_hierarchy):
        if category in base_network_filtered["road_category"].values:
            category_roads = network_mercator[
                network_mercator["road_category"] == category
            ]
            category_roads.plot(
                ax=ax,
    ...
    legend_elements = [
        Patch(
            facecolor=config.osm_road_colors[category],
    ...
    plt.savefig(
        config.output_path / "osm_road_network.png",
...
def plot_serbian_network(gdf: gpd.GeoDataFrame, config: LocalConfig) -> None:
    ...
    legend_elements = [
        Patch(
            facecolor=config.serbian_road_colors[cat], label=f"{cat}", edgecolor="none"
        )
    ...
    plt.subplots_adjust(top=0.88, bottom=0.08, left=0.02, right=0.94)
    plt.savefig(
        config.output_path / "road_categories.png",
        dpi=config.figure_dpi,
        bbox_inches="tight",
    )
    if NetworkConfig.show_figures:
...
def plot_network_comparison(
    gdf: gpd.GeoDataFrame, base_network: gpd.GeoDataFrame, config: LocalConfig
) -> None:
    """
    ...
    legend_elements2 = [
        Patch(
            facecolor=config.osm_road_colors[cat],
    ...
    plt.tight_layout()
    plt.savefig(
        config.output_path / "road_network_comparison.png",
        dpi=config.figure_dpi,
        bbox_inches="tight",
    )
    if NetworkConfig.show_figures:
...
def plot_road_length_statistics(
    ...
    length_by_category = (
        network_8682.groupby("road_category")["length_km"].sum().reset_index()
    )
    ...
    bars = ax.bar(
        ...
        color=[
            config.osm_road_colors[cat] for cat in length_by_category["road_category"]
        ],
        alpha=config.road_alpha,
        edgecolor="black",
        linewidth=0.5,
    ...
    # Customize the plot
    ax.set_title(
        f"Total Road Network Length by Category{title_suffix}",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.savefig(
        config.output_path / "road_length_by_category.png",
        dpi=config.figure_dpi,
        bbox_inches="tight",
    )
    if NetworkConfig.show_figures:
