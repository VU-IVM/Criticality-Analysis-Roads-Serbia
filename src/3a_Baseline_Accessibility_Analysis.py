...
def load_sinks(
    config: NetworkConfig, nodes: pd.DataFrame
...
def calculate_OD_matrix(
    ...
    df_agri["nearest_access_road"] = get_distance_to_nearest_facility_agriculture(
        df_agri, Sinks_road, graph
    )["closest_sink_total_fft"]
    df_agri["nearest_access_port"] = get_distance_to_nearest_facility_agriculture(
        df_agri, Sinks_port, graph
    )["closest_sink_total_fft"]
    df_agri["nearest_access_rail"] = get_distance_to_nearest_facility_agriculture(
        df_agri, Sinks_rail, graph
    )["closest_sink_total_fft"]

    return df_agri


def get_distance_to_nearest_facility_agriculture(
    df_population: gpd.GeoDataFrame, Sink: pd.DataFrame, graph: ig.Graph
) -> gpd.GeoDataFrame:
    """
...
def save_accessibilty_results(
    config: NetworkConfig, df_worldpop, Sink, facility_type
) -> None:
    """
...
def main():
    ...
    print(
        "Loading location data of border crossings, ports and rail cargo terminals..."
    )
    Sinks_road, Sinks_port, Sinks_rail, all_sinks = load_sinks(config, nodes)

    # Calculate OD matrices by sink type
    print(
        "Calculating OD matrices for agricultural areas to border crossings, ports and rail cargo stations..."
    )
