...
def merge_aadt_with_network(
    ...
    if pers_network.crs is None:
        print(
            f"  ⚠ pers_network has no CRS — assuming same as aadt_network ({aadt_network.crs})"
        )
        pers_network = pers_network.set_crs(aadt_network.crs)
    elif pers_network.crs != aadt_network.crs:
        print(
            f"  ⚠ CRS mismatch — reprojecting pers_network from {pers_network.crs} to {aadt_network.crs}"
        )
    ...
    print(
        f"  Rows after merge (before dedup) : {len(first_merger):>6} | {_nunique_deo(first_merger)}  (input was {n_input})"
    )

    # Deduplicate: for each original network row, keep max AADT values
    agg_dict = {
        col: "first" for col in pers_network.columns if col in first_merger.columns
    }
    ...
    print(
        f"  Rows after dedup                : {len(first_merger):>6} | {_nunique_deo(first_merger)}"
    )
    if len(first_merger) != n_input:
        print(
            f"  ⚠ Still {len(first_merger) - n_input:+d} rows vs input — investigate!"
        )
    ...
    print(
        f"  ✓ Matched (PA not NaN): {n_attr_matched:>6} ({100 * n_attr_matched / len(first_merger):.1f}%) | {_nunique_deo(first_merger.loc[attr_matched_mask])}"
    )
    print(
        f"  ✗ Unmatched (PA NaN)  : {n_attr_unmatched:>6} ({100 * n_attr_unmatched / len(first_merger):.1f}%) | {_nunique_deo(first_merger.loc[attr_unmatched_mask])}"
    )

    if n_attr_unmatched > 0:
        try:
            unmatched_keys = first_merger.loc[
                attr_unmatched_mask, "oznaka_deo"
            ].unique()
            print(
                f"  Sample unmatched oznaka_deo values: {unmatched_keys[:5].tolist()}"
            )
        except KeyError:
    ...
    print(
        f"  Rows into sjoin          : {n_into_sjoin:>6} | {_nunique_deo(unmatched_gdf)}"
    )
    print(
        f"  Rows out of sjoin        : {len(overlap):>6} | {_nunique_deo(overlap)}  (can be > input due to 1-to-many matches)"
    )
    print(
        f"  ✓ Got a spatial match    : {n_spatial_any:>6} ({100 * n_spatial_any / n_into_sjoin:.1f}%) | {_nunique_deo(overlap.loc[overlap['index_right'].notna()])}"
    )
    print(
        f"  ✗ No spatial match at all: {n_spatial_none:>6} ({100 * n_spatial_none / n_into_sjoin:.1f}%) | {_nunique_deo(overlap.loc[overlap['index_right'].isna()])}"
    )
    ...
    print(
        f"  Rows evaluated             : {n_evaluated:>6} | {_nunique_deo(overlap_with_aadt_geom)}"
    )
    print(
        f"  ✓ Above threshold          : {n_above:>6} ({100 * n_above / n_evaluated:.1f}%) | {_nunique_deo(overlap_with_aadt_geom.loc[above_threshold_mask])}"
    )
    print(
        f"  ✗ Below threshold (dropped): {n_below:>6} ({100 * n_below / n_evaluated:.1f}%) | {_nunique_deo(overlap_with_aadt_geom.loc[below_threshold_mask])}"
    )
    ...
    for lo, hi, label in zip(bins, bins[1:], labels):
        ...
        print(
            f"    {label:>8}  {bar:<30} {count:>5} ({100 * count / n_evaluated:.1f}%)"
        )
    ...
    print(
        f"  Rows before dropna(aadt_cols)    : {n_before_dropna:>6} | {_nunique_deo(overlap_filtered)}"
    )
    print(
        f"  ✗ Dropped (all AADT cols NaN)    : {n_dropped_no_aadt:>6} ({100 * n_dropped_no_aadt / max(n_before_dropna, 1):.1f}%) | {_nunique_deo(overlap_filtered.loc[~overlap_filtered.index.isin(overlap_for_agg.index)])}"
    )

    result = overlap_for_agg.groupby(level=0).agg(agg_dict)
    print(
        f"  Rows after groupby (deduplicated): {len(result):>6} | {_nunique_deo(result)}"
    )
    ...
    print(
        f"  Attribute-matched rows   : {len(attr_matched_rows):>6} ({100 * len(attr_matched_rows) / n_input:.1f}%) | {_nunique_deo(attr_matched_rows)}"
    )
    print(
        f"  Spatial-matched rows     : {len(result):>6} ({100 * len(result) / n_input:.1f}%) | {_nunique_deo(result)}"
    )
    ...
    print(
        f"  Leftover (no AADT at all): {len(leftovers):>6} ({100 * len(leftovers) / n_input:.1f}%) | {_nunique_deo(leftovers)}"
    )

    AADT_connected = gpd.GeoDataFrame(
        pd.concat([AADT_connected, leftovers]), geometry="geometry"
    )
    print(
        f"  Total rows after concat  : {len(AADT_connected):>6} | {_nunique_deo(AADT_connected)}"
    )
    ...
    print(
        f"  Row delta   : {row_diff:>+6}  {'✓ preserved' if row_diff == 0 else '⚠ mismatch'}"
    )
...
def create_directed_network(
    ...
    smer_counts = base_network.loc[non_oneway_mask, "smer_gdf1"].value_counts(
        dropna=False
    )
    ...
    if bad_mask.any():
        ...
        detail_cols = [
            c
            for c in ["oznaka_deo", "kategorija", "stanje", "duzina_deo"]
            if c in base_network.columns
        ]
        ...
        print(
            f"WARNING: {bad_mask.sum()} edge(s) with NaN/inf fft — inspect before proceeding:"
        )
        print(report.to_string())
        print(
            "  ^^^ Compare geom_length_m against duzina_deo to diagnose the source of the NaN geometry."
        )
    else:
...
def create_igraph_and_export(
    ...
    retained_pct = (
        100 * (1 - dropped_length / total_length) if total_length > 0 else 100.0
    )
    ...
    if retained_pct < 95.0:
        ...
        dropped_gdf = (
            base_network.loc[dropped_mask]
            .reset_index(drop=True)
            .set_crs(AADT_Serbia.crs)
        )
        dropped_roads_path = (
            NetworkConfig.intermediate_results_path
            / "giant_component_dropped_roads.parquet"
        )
        dropped_gdf.to_parquet(dropped_roads_path)
        save_excel_mirror(dropped_gdf, dropped_roads_path)
        print(
            f"  Saved {len(dropped_gdf)} dropped roads to giant_component_dropped_roads.parquet"
        )
        ...
        giant_gdf_wm.plot(
            ax=ax, color="blue", linewidth=0.8, alpha=0.7, label="Giant component"
        )
        dropped_gdf_wm.plot(
            ax=ax, color="red", linewidth=1.5, alpha=0.9, label="Dropped"
        )
        try:
            cx.add_basemap(
                ax, crs=giant_gdf_wm.crs, source=cx.providers.CartoDB.Positron
            )
        except Exception:
            pass
        ax.legend()
        ...
        ax.axis("off")
        plt.savefig(
            NetworkConfig.figure_path / "giant_component_dropped_roads.png",
            dpi=config.figure_dpi,
            bbox_inches="tight",
        )
        if NetworkConfig.show_figures:
    ...
    print(
        f"Directed graph saved to {NetworkConfig.Path_processed_road_network.resolve()}"
    )
    save_gdb_layer(
        edges_gdf,
        NetworkConfig.network_gdb,
        "main_network_directed",
