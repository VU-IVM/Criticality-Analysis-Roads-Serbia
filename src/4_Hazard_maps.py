...
def load_data(config):
    ...
    world_path = config.data_path / "ne_10m_admin_0_countries.shp"
    world = gpd.read_file(world_path)
    country_bounds = world.loc[world.SOV_A3 == 'SRB'].bounds
    country_plot = world.loc[world.SOV_A3 == 'SRB']

    # read flood data
    fluvial_flooding_path = config.data_path / "Europe_RP100_filled_depth.tif"
    ...
    return hazard_country, country_plot


def plot_flood_hazard_map(config, hazard_country):
    ...
    plt.savefig(config.figure_path / 'flood_depth_map.png', dpi=300, bbox_inches='tight')
    plt.show()

def plot_snowdrift(config, country_plot):
    snow_drift = gpd.read_file(config.data_path / "snezni_nanosi_studije.shp")

    # ===== Cell 8 =====
    # Load baseline road network
    baseline_roads = gpd.read_parquet(config.intermediate_results_path / "PERS_directed_final.parquet")
    ...
    plt.tight_layout()
    plt.savefig(config.figure_path / 'snow_drift_map.png', dpi=300, bbox_inches='tight')
    plt.show()
    

def plot_landslides(config, country_plot):
    ...
    plt.tight_layout()
    plt.savefig(config.figure_path / 'landslides_map_by_year.png', dpi=300, bbox_inches='tight')
    plt.show()
