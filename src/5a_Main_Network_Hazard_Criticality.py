...
def calculate_combined_hazard(
    ...
    gdf_hazards = gdf_hazards.loc[
        gdf_hazards[["max_depth", "dužina_sn", "datum_evid"]].any(axis=1)
    ]
...
def plot_vehicle_hours_lost_per_hazard(
    ...
    for idx, (letter, (title, gdf)) in enumerate(datasets.items()):
        ...
        country_mercator.plot(
            ax=ax, facecolor="none", edgecolor="#333333", linewidth=1.5, zorder=1
        )
    ...
    plt.tight_layout()
    plt.savefig(
        config.figure_path / "vhl_hazards_comparison.png", dpi=300, bbox_inches="tight"
    )
...
def plot_passenger_hours_lost_per_hazard(
    ...
    plt.tight_layout()
    plt.savefig(
        config.figure_path / "phl_hazards_comparison.png", dpi=300, bbox_inches="tight"
    )
    if config.show_figures:
        plt.show()
...
def plot_tonnage_kilometers_lost_per_hazard(
    ...
    plt.tight_layout()
    plt.savefig(
        config.figure_path / "tkl_hazards_comparison.png", dpi=300, bbox_inches="tight"
    )
    if config.show_figures:
        plt.show()
...
def print_analysis_summary(
    ...
    if "oznaka_deo" in gdf_vhl_flooded.columns:
        ...
        flooded_col = (
            "oznaka_deo"
            if "oznaka_deo" in gdf_vhl_flooded.columns
            else "oznaka_deo_left"
        )
        snow_col = (
            "oznaka_deo"
            if "oznaka_deo" in gdf_vhl_snowdrift.columns
            else "oznaka_deo_left"
        )
        landslide_col = (
            "oznaka_deo"
            if "oznaka_deo" in gdf_vhl_landslides.columns
            else "oznaka_deo_left"
        )
        ...
        print("\nExclusive exposure:")
        print(
            f"  Floods only: {len(flooded_segments - snow_segments - landslide_segments)}"
        )
        print(
            f"  Snow drift only: {len(snow_segments - flooded_segments - landslide_segments)}"
        )
        print(
            f"  Landslides only: {len(landslide_segments - flooded_segments - snow_segments)}"
        )

        print("\nDual exposure:")
        print(
            f"  Floods AND snow drift: {len(flooded_segments & snow_segments - landslide_segments)}"
        )
        print(
            f"  Floods AND landslides: {len(flooded_segments & landslide_segments - snow_segments)}"
        )
        print(
            f"  Snow drift AND landslides: {len(snow_segments & landslide_segments - flooded_segments)}"
        )

        print("\nTriple exposure:")
        print(
            f"  ALL three hazards: {len(flooded_segments & snow_segments & landslide_segments)}"
        )
...
def main():
    ...
    calculate_combined_hazard(
        criticality_results, flood_results, vhl_snow_drift, vhl_landslides, config
    )
    ...
    plot_vehicle_hours_lost_per_hazard(
        flood_results, vhl_snow_drift, vhl_landslides, country_plot, config
    )
    ...
    if config.print_statistics:
        print_analysis_summary(
            flood_results, vhl_snow_drift, vhl_landslides, criticality_results
        )
