...
def load_basin_data(config: NetworkConfig) -> gpd.GeoDataFrame:
    ...
    basins = pd.read_csv(
        config.intermediate_results_path
        / "SRB_flood_statistics_per_Basin_basins_scenario.csv"
    )
...
def plot_basins(basins: gpd.GeoDataFrame, config: NetworkConfig) -> None:
    ...
    plt.savefig(
        config.figure_path / "basin_water_depths.png", dpi=300, bbox_inches="tight"
    )
...
def load_road_network(config: NetworkConfig) -> gpd.GeoDataFrame:
    ...
    base_network = gpd.read_parquet(
        config.data_path / "base_network_SRB_basins.parquet"
    )
...
def calculate_hospital_accessibility(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> gpd.GeoDataFrame:
    """
    ...
    hospital_basins = (
        config.accessibility_analysis_path
        / "healthcare_criticality_results"
        / "unique_scenarios_SRB_basins.pkl"
    )
    ...
    exposed_edges_filtered = (
        exposed_edges[exposed_edges["travel_time_impact"] >= 0.167].copy().to_crs(3857)
    )
...
def plot_road_criticality_hospital_access(
    base_network: gpd.GeoDataFrame,
    exposed_edges_filtered: gpd.GeoDataFrame,
    config: NetworkConfig,
) -> None:
    """
    ...
    plt.subplots_adjust(top=0.88, bottom=0.08, left=0.02, right=0.94)
    plt.savefig(
        config.figure_path / "hospital_criticality.png", dpi=200, bbox_inches="tight"
    )
    if config.show_figures:
        plt.show()


def calculate_police_accessibility(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> gpd.GeoDataFrame:
    """
...
def plot_road_criticality_map_emergency_service(
    exposed_edges_filtered: gpd.GeoDataFrame,
    base_network: gpd.GeoDataFrame,
    config: NetworkConfig,
    emergency_service: Any,
) -> None:
    """
...
def calculate_firefigher_accessibility(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> gpd.GeoDataFrame:
    """
...
def calculate_criticality_factory_access(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> gpd.GeoDataFrame:
    """
    ...
    # Filter out everything below 10 minutes (0.167 hours)
    factory_exposed_edges = (
        factory_exposed_edges[factory_exposed_edges["travel_time_impact"] >= 0.167]
        .copy()
        .to_crs(3857)
    )
...
def plot_road_criticality_factories(
    ...
    plt.subplots_adjust(top=0.88, bottom=0.08, left=0.02, right=0.94)
    plt.savefig(
        config.figure_path / "factory_criticality.png", dpi=200, bbox_inches="tight"
    )
...
def calculate_road_criticality_agriculture(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> dict:
    """
    ...
    for sink_type in sink_types:
        ...
        exposed_edges = exposed_edges.loc[exposed_edges.exposed].reset_index(
            drop=True
        )
...
def plot_panel(
    ...
    if gdf is not None and len(gdf) > 0:
        for category, color in zip(labels, colors):
            ...
            if len(subset) > 0:
                subset.plot(
                    ax=ax, color=color, linewidth=width_mapping[category], alpha=0.9
                )
...
def plot_agriculture_average_increased_travel_time(
    exposed_edges_by_type: gpd.GeoDataFrame,
    base_network: gpd.GeoDataFrame,
    config: NetworkConfig,
) -> None:
    """
    ...
    # Plot panels
    plot_panel(
        axes[0], exposed_edges_by_type["road"], "A", sink_titles["road"], base_network
    )
    plot_panel(
        axes[1], exposed_edges_by_type["port"], "B", sink_titles["port"], base_network
    )
    plot_panel(
        axes[2], exposed_edges_by_type["rail"], "C", sink_titles["rail"], base_network
    )
    ...
    # Save figure
    output_fig_path = config.figure_path / "SRB_agri_criticality_avg_3x1.png"
    plt.savefig(output_fig_path, dpi=150, bbox_inches="tight")
    print(
        "Average increased travel time from agricultural areas to A: road borders, B: ports, C: rail terminals."
    )
    if config.show_figures:
        plt.show()


def plot_agriculture_increased_travel_time_to_nearest(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> None:
    """
    ...
    # save as .parquet
    exposed_edges_by_type["road"].to_parquet(
        config.intermediate_results_path / "road_impacts.parquet"
    )
    exposed_edges_by_type["rail"].to_parquet(
        config.intermediate_results_path / "rail_impacts.parquet"
    )
    exposed_edges_by_type["port"].to_parquet(
        config.intermediate_results_path / "port_impacts.parquet"
    )

    # save as .gpkg
    exposed_edges_by_type["road"].to_file(
        config.intermediate_results_path / "road_impacts.gpkg", driver="GPKG"
    )
    exposed_edges_by_type["rail"].to_file(
        config.intermediate_results_path / "rail_impacts.gpkg", driver="GPKG"
    )
    exposed_edges_by_type["port"].to_file(
        config.intermediate_results_path / "port_impacts.gpkg", driver="GPKG"
    )
    ...
    # Save figure
    output_fig_path = config.figure_path / "SRB_agri_criticality_nearest_3x1.png"
    plt.savefig(output_fig_path, dpi=150, bbox_inches="tight")
    print(
        "Increased travel time from agricultural areas to nearest A: road border, B: port, C: rail terminal."
    )
    if config.show_figures:
        plt.show()


def plot_and_save_combined_results(
    base_network: gpd.GeoDataFrame, config: NetworkConfig
) -> None:
    """
    ...
    # Process all
    hospital_exposed_edges = process_service(
        hospital_results_path, hospital_basins_path
    )
    ...
    plt.savefig(
        config.figure_path / "criticality_2x2.png", dpi=150, bbox_inches="tight"
    )
    print(
        "combined results for increased travel time to A: hospitals, B: factories, C: police stations, D; fire stations."
    )
    if config.show_figures:
        plt.show()
    ...
    def panel_facts(gdf):
        ...
        def get(k):
            return float(counts.get(k, 0) / total * 100.0)
    ...
    panel_facts(hospital_exposed_edges)  # Figure 25.A
    panel_facts(factory_exposed_edges)  # Figure 25.B
    panel_facts(police_exposed_edges)  # Figure 25.C
    panel_facts(fire_exposed_edges)  # Figure 25.D
    ...
    if frames:
        ...
        float(all_delays_h.mean() * 60.0)
        float(all_delays_h.median() * 60.0)
        float(pd.concat(frames).geometry.length.sum() / 1000.0)
    else:
        pass
    ...
    def r(x, nd=0):
        return (
            int(round(x))
            if nd == 0 and pd.notna(x)
            else (round(x, nd) if pd.notna(x) else None)
        )

    # save to .parquet
    hospital_exposed_edges.to_parquet(
        config.intermediate_results_path / "hospital_impacts.parquet"
    )
    factory_exposed_edges.to_parquet(
        config.intermediate_results_path / "factory_impacts.parquet"
    )
    police_exposed_edges.to_parquet(
        config.intermediate_results_path / "police_impacts.parquet"
    )
    fire_exposed_edges.to_parquet(
        config.intermediate_results_path / "fire_impacts.parquet"
    )

    # save to .gpkg
    hospital_exposed_edges.to_file(
        config.intermediate_results_path / "hospital_impacts.gpkg", driver="GPKG"
    )
    factory_exposed_edges.to_file(
        config.intermediate_results_path / "factory_impacts.gpkg", driver="GPKG"
    )
    police_exposed_edges.to_file(
        config.intermediate_results_path / "police_impacts.gpkg", driver="GPKG"
    )
    fire_exposed_edges.to_file(
        config.intermediate_results_path / "fire_impacts.gpkg", driver="GPKG"
    )
...
def main():
    ...
    plot_road_criticality_map_emergency_service(
        road_criticality_hospitals, base_network, config, "hospitals"
    )

    road_criticality_police = calculate_police_accessibility(base_network, config)
    plot_road_criticality_map_emergency_service(
        road_criticality_police, base_network, config, "police"
    )

    road_criticality_firefighters = calculate_firefigher_accessibility(
        base_network, config
    )
    plot_road_criticality_map_emergency_service(
        road_criticality_firefighters, base_network, config, "firefighters"
    )
    ...
    road_criticality_factories = calculate_criticality_factory_access(
        base_network, config
    )
    ...
    road_criticality_agriculture = calculate_road_criticality_agriculture(
        base_network, config
    )
    plot_agriculture_average_increased_travel_time(
        road_criticality_agriculture, base_network, config
    )
