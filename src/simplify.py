...
def split_multilinestrings(network):
    ...
    for edge in tqdm(
        ...
        if shapely.get_type_id(edge.geometry) == 5:
            ...
            edge_parts = [
                x
                for x in shapely.geometry.get_geometry(
                    edge, shapely.geometry.get_num_geometries(edge)
                )
            ]
        else:
    ...
    edges = (
        pd.concat(simple_edge_attrs, axis=0)
...
def calculate_degree(network):
    ...
    if ndC - 1 > max(network.edges.from_id) and ndC - 1 > max(network.edges.to_id):
        print(
            "Warning: calculate_degree may encounter issues with the network structure."
        )

    # Calculate the connectivity degree using numpy bincount
    return np.bincount(network.edges["from_id"], minlength=ndC) + np.bincount(
        network.edges["to_id"], minlength=ndC
    )
...
def merge_edges(network, print_err=False):
    ...
    while n2:
        ...
        if len(eID) > 2:
            edgePath1, edgePath2 = find_closest_2_edges(eID, edg, node_geometry)
        elif len(eID) < 2:
            continue
        else:
        ...
        nextNode1 = (
            edgePath1.to_id if edgePath1.from_id == nodeID else edgePath1.from_id
        )
        nextNode2 = (
            edgePath2.to_id if edgePath2.from_id == nodeID else edgePath2.from_id
        )
        ...
        while deg[nextNode1] == 2:
            ...
            nextNode1 = (
                edgePath1.to_id if edgePath1.from_id == nextNode1 else edgePath1.from_id
            )
            newEdge.append(edgePath1.geometry)
        ...
        while deg[nextNode2] == 2:
            ...
            nextNode2 = (
                edgePath2.to_id if edgePath2.from_id == nextNode2 else edgePath2.from_id
            )
            newEdge.append(edgePath2.geometry)
            possibly_delete.append(edgePath2.id)
        # Update the information of the first edge
        new_merged_geom = shapely.line_merge(
            shapely.multilinestrings([x for x in newEdge])
        )
        if shapely.get_type_id(new_merged_geom) == 1:
            ...
            edg.loc[info_first_edge, optional_cols] = (
...
def snap_line(line, points, tolerance=1e-9):
    ...
    if shapely.get_type_id(line.geometry) == 0:
        ...
        points = [
            point for point in points if shapely.distance(point, line) < tolerance
        ]
        for point in points:
            line = shapely.snap(line, points, tolerance=1e-9)
    return line
...
def nearest_network_node_list(gdf_admin, gdf_nodes, sg):
    ...
    for admin_ in gdf_admin.itertuples():
        ...
        #     continue
        nodes[admin_.id] = gdf_nodes.iloc[
            shapely.distance((admin_.geometry), gdf_nodes.geometry).idxmin()
        ].id
    return nodes
...
def split_edges_at_nodes(network, tolerance=1e-9, attributes=[]):
    ...
    for iter_, edge in tqdm(network.edges.iterrows(), total=len(network.edges)):
        ...
        if isinstance(hits_edges, shapely.MultiLineString) | isinstance(
            hits_edges, shapely.LineString
        ):
            hits_edges = []
        else:
            overlapping_points = [
                x
                for x in edge.geometry.intersection(hits_edges)
                if isinstance(x, shapely.Point)
            ]

        #### combine the overlapping points and keep only the unique ones
        all_points_combined = shapely.extract_unique_points(
            shapely.MultiPoint(overlapping_points + hits_nodes)
        )
        ...
        grab_all_edges.append(
            pd.DataFrame(
                [list(edge[attributes].values) + [x] for x in new_edges],
                columns=network.edges.columns,
            )
        )

    # return new network with split edges
    return Network(
        nodes=network.nodes, edges=gpd.GeoDataFrame(pd.concat(grab_all_edges))
    )
...
def fill_attributes(network):
    ...
    vals_to_assign.maxspeed = vals_to_assign.apply(
        lambda x: fill_empty_maxspeed(x), axis=1
    )

    # Get the maximum value from the maxspeed column
    vals_to_assign.maxspeed = vals_to_assign.maxspeed.apply(
        lambda x: get_max_in_vals_to_assign(x)
    )
...
def ferry_connected_network(country, data_path, tqdm_on=True):
    ...
    # load full network
    full_network = roads(
        str(data_path.joinpath("country_osm", "{}.osm.pbf".format(country)))
    )
    main_road_network = full_network.loc[
        full_network.highway.isin(road_types)
    ].reset_index(drop=True)

    # load ferries
    ferry_network = ferries(
        str(data_path.joinpath("country_osm", "{}.osm.pbf".format(country)))
    )

    # create a main network where hanging nodes are not removed
    network_with_hanging = simplified_network(
        main_road_network, drop_hanging_nodes_run=False
    )
    ...
    for link in connectors.itertuples():
        ...
        nodes.id.loc[nodes.geometry == start]
        nodes.id.loc[nodes.geometry == end]
        edges = edges.append(
            {
    ...
    new_edges = new_edges[
        [x for x in new_edges.columns if x != "geometry"] + ["geometry"]
    ]
    ...
    net_final.edges.to_feather(
        data_path.joinpath("road_ferry_networks", "{}-edges.feather".format(country))
    )
    net_final.nodes.to_feather(
        data_path.joinpath("road_ferry_networks", "{}-nodes.feather".format(country))
    )
...
def connect_ferries(country, full_network, ferry_network):
    ...
    for iter_, ferry in ferry_network.iterrows():
        ...
        sub_full_network = full_network.loc[
            shapely.intersects(full_network.geometry, ferry_buffer)
        ].reset_index(drop=True)
        sub_main_network_nodes = [
            [
                shapely.points(shapely.get_coordinates(x)[0]),
                shapely.points(shapely.get_coordinates(x)[1]),
            ]
            for x in sub_full_network.loc[
                sub_full_network.highway.isin(road_types)
            ].geometry
        ]
        sub_main_network_nodes = [
            item for sublist in sub_main_network_nodes for item in sublist
        ]
        sub_main_network_nodes = gpd.GeoDataFrame(
            sub_main_network_nodes, columns=["geometry"]
        )
        sub_main_network_nodes["id"] = [
            x + 1 for x in range(len(sub_main_network_nodes))
        ]

        # create a dataframe of the ferry nodes
        ferry_nodes = gpd.GeoDataFrame(
            [
                shapely.points(shapely.get_coordinates(ferry.geometry)[0]),
                shapely.points(shapely.get_coordinates(ferry.geometry)[-1]),
            ],
            columns=["geometry"],
        )
        ferry_nodes["id"] = [1, 2]
        ...
        edges = net.edges.reindex(
            ["from_id", "to_id"]
            + [x for x in list(net.edges.columns) if x not in ["from_id", "to_id"]],
            axis=1,
        )
        ...
        nearest_node_main = nearest_network_node_list(
            sub_main_network_nodes, net.nodes, sg
        )
        nearest_node_ferry = nearest_network_node_list(ferry_nodes, net.nodes, sg)
        dest_nodes = [
            sg.vs["name"].index(nearest_node_main[x])
            for x in list(nearest_node_main.keys())
        ]
        ferry_nodes_graph = [
            sg.vs["name"].index(nearest_node_ferry[x])
            for x in list(nearest_node_ferry.keys())
        ]
        ...
        if len(ferry_nodes_graph) == 2:
            ...
            for dest_node in dest_nodes:
                ...
                if len(paths[0]) != 0:
                    collect_start_paths[dest_node] = (
                        sg.es[paths[0]]["id"],
                        np.sum(sg.es[paths[0]]["distance"]),
                    )

            start_coords = ferry_nodes.geometry[
                ferry_nodes.id
                == {v: k for k, v in nearest_node_ferry.items()}[
                    sg.vs[start_node]["name"]
                ]
            ].values
            ...
            if len(collect_start_paths) != 0:
                if len(gpd.GeoDataFrame.from_dict(collect_start_paths).T.min()) != 0:
                    ...
                    if len(p_1) > 0:
                        linestring = shapely.linear.line_merge(
                            shapely.multilinestrings(path_1["geometry"].values)
                        )

                        endpoint1 = shapely.points(
                            shapely.coordinates.get_coordinates(linestring)
                        )[0]
                        endpoint2 = shapely.points(
                            shapely.coordinates.get_coordinates(linestring)
                        )[-1]
                        ...
                        local_network = net.edges.loc[
                            net.edges.id.isin(
                                gpd.GeoDataFrame.from_dict(collect_start_paths).T.min()[
                                    0
                                ]
                            )
                        ]
                        sub_local_network = [
                            ...
                            for x in local_network.loc[
                                local_network.highway.isin(road_types)
                            ].geometry
                        ]
                        sub_local_network = [
                            item for sublist in sub_local_network for item in sublist
                        ]
                        location_closest_point = np.where(
                            shapely.distance(start_coords[0], sub_local_network)
                            == np.amin(
                                shapely.distance(start_coords[0], sub_local_network)
                            )
                        )[0][0]
                        collect_connectors.append(
                            shapely.linestrings(
                                np.concatenate(
                                    (
                                        shapely.get_coordinates(start_coords),
                                        shapely.get_coordinates(
                                            sub_local_network[location_closest_point]
                                        ),
                                    ),
                                    axis=0,
                                )
            ...
            # if there are no paths, but if the ferry node is still very close to the main network, we create a new linestring to connect them up (sometimes the ferry dock has no road)
                ...
                shapely.distance(sub_main_network_nodes.geometry, start_coords).min()
                < 0.01
            ):
                get_new_end_point = shapely.coordinates.get_coordinates(
                    sub_main_network_nodes.iloc[
                        shapely.distance(
                            sub_main_network_nodes.geometry, start_coords
                        ).idxmin()
                    ].geometry
                )
                collect_connectors.append(
                    shapely.linestrings(
                        np.concatenate(
                            (
                                shapely.coordinates.get_coordinates(start_coords),
                                get_new_end_point,
                            ),
                            axis=0,
                        )
            ...
            for dest_node in dest_nodes:
                ...
                if len(paths[0]) != 0:
                    collect_end_paths[dest_node] = (
                        sg.es[paths[0]]["id"],
                        np.sum(sg.es[paths[0]]["distance"]),
                    )

            end_coords = ferry_nodes.geometry[
                ferry_nodes.id
                == {v: k for k, v in nearest_node_ferry.items()}[
                    sg.vs[end_node]["name"]
                ]
            ...
            if len(collect_end_paths) != 0:
                if len(gpd.GeoDataFrame.from_dict(collect_end_paths).T.min()) != 0:
                    ...
                    if len(p_2) > 0:
                        linestring = shapely.linear.line_merge(
                            shapely.multilinestrings(path_2["geometry"].values)
                        )
                        ...
                        local_network = net.edges.loc[
                            net.edges.id.isin(
                                gpd.GeoDataFrame.from_dict(collect_end_paths).T.min()[0]
                            )
                        ]
                        sub_local_network = [
                            [
                        ...
                        location_closest_point = np.where(
                            shapely.distance(end_coords[0], sub_local_network)
                            == np.amin(
                                shapely.distance(end_coords[0], sub_local_network)
                            )
                        )[0][0]
                        collect_connectors.append(
                            shapely.linestrings(
                                np.concatenate(
                                    (
                                        shapely.get_coordinates(end_coords),
                                        shapely.get_coordinates(
                                            sub_local_network[location_closest_point]
                                        ),
                                    ),
            ...
            # if there are no paths, but if the ferry node is still very close to the main network, we create a new linestring to connect them up (sometimes the ferry dock has no road)
                ...
                shapely.distance(sub_main_network_nodes.geometry, end_coords).min()
                < 0.01
            ):
                get_new_end_point = shapely.coordinates.get_coordinates(
                    sub_main_network_nodes.iloc[
                        shapely.distance(
                            sub_main_network_nodes.geometry, end_coords
                        ).idxmin()
                    ].geometry
                )
                collect_connectors.append(
                    shapely.linestrings(
                        np.concatenate(
                            (
                                shapely.coordinates.get_coordinates(end_coords),
                                get_new_end_point,
                            ),
                            axis=0,
                        )
        ...
        # collect paths on one side  of the ferry, as other side does not have a network nearby
            ...
            if len(collect_start_paths) != 0:
                ...
                if len(p_1) > 0:
                    ...
                    local_network = net.edges.loc[
                        net.edges.id.isin(
                            gpd.GeoDataFrame.from_dict(collect_start_paths).T.min()[0]
                        )
                    ]
                    sub_local_network = [
                        [
                    ...
                    location_closest_point = np.where(
                        shapely.distance(start_coords[0], sub_local_network)
                        == np.amin(shapely.distance(start_coords[0], sub_local_network))
                    )[0][0]
                    collect_connectors.append(
                        shapely.linestrings(
                            np.concatenate(
            ...
            # if there are no paths, but if the ferry node is still very close to the main network, we create a new linestring to connect them up (sometimes the ferry dock has no road)
                ...
                get_new_end_point = shapely.get_coordinates(
                    sub_main_network_nodes.iloc[
                        shapely.distance(
                            sub_main_network_nodes.geometry, start_coords
                        ).idxmin()
                ...
                collect_connectors.append(
                    shapely.linestrings(
                        np.concatenate(
                            (shapely.get_coordinates(start_coords), get_new_end_point),
                            axis=0,
                        )
                    )
                )
