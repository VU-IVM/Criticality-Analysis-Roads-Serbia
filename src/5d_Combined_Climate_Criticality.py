...
def add_impact_column(
    ...
    agg="mean",
    col_to_focus="travel_time_impact",
) -> gpd.GeoDataFrame:
    """
...
def load_and_preprocess_criticality_data(config: NetworkConfig) -> gpd.GeoDataFrame:
    ...
    factory_exposed_edges = gpd.read_parquet(config.Path_factory_impacts).to_crs(
        gdf_hazards.crs
    )
    police_exposed_edges = gpd.read_parquet(config.Path_police_impacts).to_crs(
        gdf_hazards.crs
    )
    fire_exposed_edges = gpd.read_parquet(config.Path_fire_fighter_impacts).to_crs(
        gdf_hazards.crs
    )
    border_exposed_edges = gpd.read_parquet(config.Path_road_border_impacts).to_crs(
        gdf_hazards.crs
    )
    port_exposed_edges = gpd.read_parquet(config.Path_port_impacts).to_crs(
        gdf_hazards.crs
    )
    railway_exposed_edges = gpd.read_parquet(config.Path_railway_impacts).to_crs(
        gdf_hazards.crs
    )

    # Load results of climate change analysis
    future_flood_change_rp = gpd.read_parquet(
        config.Path_future_floods_change_RP
    ).to_crs(gdf_hazards.crs)
    future_rainfall_change = gpd.read_parquet(
...
def plot_climate_criticality_components(
    gdf_hazards: gpd.GeoDataFrame, config: NetworkConfig
) -> None:
    """
    ...
    # Save or show
    plt.savefig(
        config.figure_path / "criticality_analysis_3panel.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def plot_combined_climate_criticality(
    ...
    plt.tight_layout()
    plt.savefig(
        config.figure_path / "climate_criticality_mean.png",
        dpi=150,
        bbox_inches="tight",
    )
    if config.show_figures:
...
def print_statistics(gdf_hazards: gpd.GeoDataFrame, config: NetworkConfig) -> None:
    ...
    for class_col, class_name in zip(class_cols, class_names):
        ...
        ct = (
            pd.crosstab(
                gdf_hazards["kategorija"], gdf_hazards[class_col], normalize="index"
            )
            * 100
        )
    ...
    tier_ct = (
        pd.crosstab(
            gdf_hazards["road_tier"], gdf_hazards["mean_class"], normalize="index"
        )
        * 100
    )
    ...
    high_tier_scores = gdf_hazards[gdf_hazards["road_tier"] == "High-Tier (IA/IM/IB)"][
        "criticality_mean"
    ]
    low_tier_scores = gdf_hazards[gdf_hazards["road_tier"] == "Low-Tier (IIA/IIB)"][
        "criticality_mean"
    ]
    ...
    for class_col, class_name in zip(class_cols, class_names):
        ...
        high_crit_pct = (
            gdf_hazards.groupby("kategorija")
            .apply(lambda x: (x[class_col].isin(high_crit_labels)).sum() / len(x) * 100)
        ...
        for cat in high_crit_pct.index:
            ...
            count_high = gdf_hazards[
                (gdf_hazards["kategorija"] == cat)
                & (gdf_hazards[class_col].isin(high_crit_labels))
            ].shape[0]
    ...
    for cat in gdf_hazards["kategorija"].dropna().unique():
        ...
        pct_CC_high = (
            (subset["mean_class"].isin(high_crit_labels)).sum() / n_sections * 100
        )
    ...
    print(
        f"  Section: {top1.get('naziv_poce', 'N/A')} → {top1.get('naziv_zavr', 'N/A')}"
    )
