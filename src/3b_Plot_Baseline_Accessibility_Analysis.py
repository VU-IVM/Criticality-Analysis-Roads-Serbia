...
def load_accessibility_results(
    config: NetworkConfig, facility_type
...
def plot_access_curve(
    df_worldpop: gpd.GeoDataFrame, config: NetworkConfig, emergency_service
) -> None:
    """
...
def plot_access_times_factories(
    df_factories: pd.DataFrame, Sink: pd.DataFrame, config: NetworkConfig
) -> None:
    """
    ...
    Sink_plot = gpd.GeoDataFrame(Sink, geometry="geometry", crs="EPSG:4326").to_crs(
        3857
    )
    ...
    plt.savefig(
        config.figure_path / "factory_access_avg.png", dpi=200, bbox_inches="tight"
    )
    if config.show_figures:
        plt.show()
...
def plot_accessibility_curves_agriculture(
    df_agri: pd.DataFrame, config: NetworkConfig
) -> None:
    """
    ...
    thresholds = np.arange(
        0, 8.1, 0.5
    )  # Extended range based on your data (up to ~5.8h)
    ...
    plt.tight_layout()
    plt.savefig(
        config.figure_path / "baseline_accessibility_agri_road_port_rail_3x2.png",
        dpi=150,
        transparent=True,
    )
    if config.show_figures:
...
def plot_access_time_agriculture_map(
    df_agri: pd.DataFrame, Sinks: pd.DataFrame, config: NetworkConfig
) -> None:
    """
    ...
    Sinks_plot = gpd.GeoDataFrame(Sinks, geometry="geometry", crs="EPSG:4326").to_crs(
        3857
    )
...
def plot_access_time_map(
    ...
    df_worldpop_plot = gpd.GeoDataFrame(
        df_worldpop, geometry="geometry", crs="EPSG:4326"
    ).to_crs(3857)
    Sink_fire = gpd.GeoDataFrame(Sink, geometry="geometry", crs="EPSG:4326").to_crs(
        3857
    )
...
def plot_accessibility_chart(
    df_worldpop_plot: gpd.GeoDataFrame, config: NetworkConfig, emergency_service
) -> None:
    """
...
def main():
    ...
    plot_access_time_agriculture_map(
        df_agriculture_accessibility, df_agriculture_sinks, config
    )
    ...
    df_worldpop_plot = plot_access_time_map(
        df_firefighter_accessibility, df_fire_stations, config, "firefighters"
    )
    ...
    # Plot a map that shows the accessibility time of each settlement to the clostest fire station
    df_worldpop_plot = plot_access_time_map(
        df_hospital_accessibility, df_hospitals, config, "hospitals"
    )
    ...
    # Plot a map that shows the accessibility time of each settlement to the clostest fire station
    df_worldpop_plot = plot_access_time_map(
        df_police_accessibility, df_police, config, "police"
    )
