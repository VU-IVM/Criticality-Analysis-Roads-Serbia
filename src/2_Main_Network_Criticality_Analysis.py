...
class CriticalityConfig:
    ...
    disruption_colors: List[str] = field(
        default_factory=lambda: ["#fee5d9", "#fcae91", "#fb6a4a", "#de2d26", "#a50f15"]
    )

    # PHL/THL/PKL/TKL colour ramp (pink palette)
    person_colors: List[str] = field(
        default_factory=lambda: ["#fcc5c0", "#fa9fb5", "#f768a1", "#c51b8a", "#7a0177"]
    )

    # Classification bins / labels / linewidths
    time_bins: List[float] = field(
        default_factory=lambda: [0, 0.08335, 0.25, 0.5, 1, np.inf]
    )
    ...
    time_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "No Delay": 0.5,
            "5-15 min": 1.5,
            "15-30 min": 2.0,
    ...
    vhl_bins: List[float] = field(
        default_factory=lambda: [0, 1000, 5000, 10_000, 25_000, np.inf]
    )
    vhl_labels: List[str] = field(
        default_factory=lambda: ["0-1K", "1K-5K", "5K-10K", "10K-25K", "25K+"]
    )
    vhl_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "0-1K": 0.5,
            "1K-5K": 1.5,
            "5K-10K": 2.0,
    ...
    dist_labels: List[str] = field(
        default_factory=lambda: ["0-5 km", "5-15 km", "15-30 km", "30-60 km", "60+ km"]
    )
    dist_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "0-5 km": 0.5,
            "5-15 km": 1.5,
            "15-30 km": 2.0,
    ...
    vkl_bins: List[float] = field(
        default_factory=lambda: [0, 10_000, 50_000, 100_000, 250_000, np.inf]
    )
    vkl_labels: List[str] = field(
        default_factory=lambda: ["0-10K", "10K-50K", "50K-100K", "100K-250K", "250K+"]
    )
    vkl_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "0-10K": 0.5,
            "10K-50K": 1.5,
            "50K-100K": 2.0,
    ...
    phl_bins: List[float] = field(
        default_factory=lambda: [0, 1000, 5000, 10_000, 25_000, np.inf]
    )
    phl_labels: List[str] = field(
        default_factory=lambda: ["0-1K", "1K-5K", "5K-10K", "10K-25K", "25K+"]
    )
    phl_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "0-1K": 0.5,
            "1K-5K": 1.5,
            "5K-10K": 2.0,
    ...
    thl_bins: List[float] = field(
        default_factory=lambda: [0, 500, 2500, 5000, 10_000, np.inf]
    )
    thl_labels: List[str] = field(
        default_factory=lambda: ["0-500", "500-2.5K", "2.5K-5K", "5K-10K", "10K+"]
    )
    thl_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "0-500": 0.5,
            "500-2.5K": 1.5,
            "2.5K-5K": 2.0,
    ...
    pkl_bins: List[float] = field(
        default_factory=lambda: [10_000, 50_000, 100_000, 250_000, 500_000, np.inf]
    )
    pkl_labels: List[str] = field(
        default_factory=lambda: [
            "10K-50K",
            "50K-100K",
            "100K-250K",
    ...
    pkl_linewidths: Dict[str, float] = field(
        default_factory=lambda: {
            "10K-50K": 0.5,
            "50K-100K": 1.5,
            "100K-250K": 2.0,
...
def load_population_data(config: CriticalityConfig) -> gpd.GeoDataFrame:
    ...
    geometry = [
        Point(lon, lat) for lon, lat in zip(df_clean["longitude"], df_clean["latitude"])
    ]
    ...
    nodes = pd.concat(
        [
            pd.DataFrame.from_dict(
                dict(zip(edges["to_id"], to_geom)), orient="index", columns=["geometry"]
            ),
            pd.DataFrame.from_dict(
                dict(zip(edges["from_id"], from_geom)),
                orient="index",
                columns=["geometry"],
            ),
        ]
...
def _build_sister_pairs(graph: ig.Graph) -> Dict[int, Optional[int]]:
    ...
    return {
        edge.index: edge_lookup.get((edge.target, edge.source)) for edge in graph.es
    }
...
def summarise_od(
    ...
    total_trips = (
        baseline_time.shape[0] * baseline_time.shape[1]
    ) - baseline_time.shape[0]
    ...
    delayed_mask = (np.array(od_time) != np.array(baseline_time)) & (
        ~np.isnan(np.array(od_time))
    )
    np.array(od_time) == config.fail_value
    ...
    if len(delayed_times) > 0 and delayed_demands.sum() > 0:
        average_time_disruption = float(
            np.average(delayed_times, weights=delayed_demands)
        )
        average_dist_disruption = float(
            np.average(delayed_dists, weights=delayed_demands)
        )
    else:
...
def plot_spof_disruption_maps(
    ...
    _plot_classified_map(
        ...
        config.vkl_bins,
        config.vkl_labels,
        config.vkl_linewidths,
        c,
        "D",
        "Vehicle Kilometers Lost",
        legend_suffix=" km",
    )
...
def plot_spof_person_maps(
    ...
    _plot_classified_map(
        ...
        config.thl_bins,
        config.thl_labels,
        config.thl_linewidths,
        c,
        "B",
        "Tonnage Hours Lost",
        legend_suffix=" ton-hours",
    )
    _plot_classified_map(
        axes[1, 0],
        gdf_results,
    ...
    _plot_classified_map(
        ...
        config.tkl_bins,
        config.tkl_labels,
        config.tkl_linewidths,
        c,
        "D",
        "Tonnage Kilometers Lost",
        legend_suffix=" ton-km",
    )
...
def plot_road_type_boxplots(
    ...
    gdf_results.boxplot(
        column="average_time_disruption", by="kategorija", ax=axes[0, 0]
    )
    ...
    gdf_results.boxplot(
        column="average_dist_disruption", by="kategorija", ax=axes[1, 0]
    )
...
def plot_road_type_violins(
    ...
    pairs = [
        ...
        (axes[0, 1], "phl", "Passenger Hours", "B. Passenger Hours Lost by Road Type"),
        (
            axes[1, 0],
            "average_dist_disruption",
            "Kilometers",
...
def plot_aadt_vs_criticality(
    ...
    sc1 = axes[0].scatter(
        gdf_results["total_aadt"],
        gdf_results["phl"],
        c=gdf_results["average_time_disruption"],
        cmap="Reds",
        alpha=0.6,
        s=20,
    )
    ...
    sc2 = axes[1].scatter(
        gdf_results["total_aadt"],
        gdf_results["pkl"],
        c=gdf_results["average_dist_disruption"],
        cmap="Reds",
        alpha=0.6,
        s=20,
    )
...
def print_summary_statistics(gdf_results: gpd.GeoDataFrame) -> None:
    ...
    d_5_15 = (
        gdf_results["average_time_disruption"].between(0, 0.25, inclusive="right").sum()
    )
    d_15_30 = (
        gdf_results["average_time_disruption"]
        .between(0.25, 0.5, inclusive="right")
        .sum()
    )
    d_30_60 = (
        gdf_results["average_time_disruption"]
        .between(0.5, 1.0, inclusive="right")
        .sum()
    )
    ...
    print("=" * 60)
    road_summary = (
        gdf_results.groupby("kategorija")
        .agg(
            count=("edge_no", "count"),
